...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
  ...
  if len(to_send) > 2000:
      await ctx.send(f"<:tick:1348326381611647046> | Successfully removed {deleted} messages.", delete_after=3)
  else:
...
class Owner(commands.Cog):
    ...
    def __init__(self, client):
        ...
        self.bot_owner_ids = [1005088956951564358, 1005088956951564358]
    ...
    async def staff_add(self, ctx, user: discord.User):
        if user.id in self.staff:
            codex = discord.Embed(title="<:alert:1348340453803687966> Access Denied", description=f"{user} is already in the staff list.", color=0x000000)
            await ctx.reply(embed=codex, mention_author=False)
        else:
            self.staff.add(user.id)
            async with aiosqlite.connect(self.db_path) as db:
                await db.execute('INSERT OR IGNORE INTO staff (id) VALUES (?)', (user.id,))
                await db.commit()
            codex2 = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Added {user} to the staff list.", color=0x000000)
    ...
    async def staff_remove(self, ctx, user: discord.User):
        if user.id not in self.staff:
            codex = discord.Embed(title="<:alert:1348340453803687966> Access Denied", description=f"{user} is not in the staff list.", color=0x000000)
            await ctx.reply(embed=codex, mention_author=False)
        else:
            ...
            async with aiosqlite.connect(self.db_path) as db:
                ...
                await db.commit()
                codex2 = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Removed {user} from the staff list.", color=0x000000)
            await ctx.reply(embed=codex2, mention_author=False)
    ...
    async def staff_list(self, ctx):
        if not self.staff:
            ...
            staff_display = "\n".join(member_list)
            codex = discord.Embed(title="<:automod:1348326413912248432> CodeX Staffs", description=f"\n{staff_display}", color=0x000000)
            await ctx.send(embed=codex)
    ...
    async def _slist(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            description="",
            title=f"Guild List of CodeX [{len(self.client.guilds)}]",
            color=0x000000,
    ...
    @commands.command(name="CodeX.restart", help="Restarts the client.")
    @commands.is_owner()
    async def _restart(self, ctx: Context):
        await ctx.reply("Restarting CodeX...")
        restart_program()
    ...
    async def own_list(self, ctx):
        ...
        embeds = DescriptionEmbedPaginator(
            entries=entries,
            title=f"CodeX Owners [{len(nplist)}]",
            description="",
            per_page=10,
    ...
    async def dm(self, ctx, user: discord.User, *, message: str):
        ...
        try:
            ...
            await ctx.send(f"<:tick:1348326381611647046> | Successfully Sent a DM to **{user}**")
        except discord.Forbidden:
    ...
    async def change_nickname(self, ctx, *, name: str = None):
        ...
        try:
            ...
            if name:
                await ctx.send(f"<:tick:1348326381611647046> | Successfully changed nickname to **{name}**")
            else:
                await ctx.send("<:tick:1348326381611647046> | Successfully removed nickname")
        ...
        if member:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:tick:1348326381611647046> | **{member.name}** has been successfully banned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000)
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:alert:1348340453803687966> I do not have permission to ban **{member.name}** in this guild.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:alert:1348340453803687966> An error occurred while banning **{member.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False, delete_after=5)
                await ctx.message.delete()
        ...
        if user:
            try:
                ...
                embed = discord.Embed(
                    ...
                    description=f"<:tick:1348326381611647046> | **{user.name}** has been successfully unbanned from {ctx.guild.name} by the Bot Owner.",
                    color=0x000000
                )
                ...
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:alert:1348340453803687966> I do not have permission to unban **{user.name}** in this guild.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
            except discord.HTTPException:
                embed = discord.Embed(
                    title="Error!",
                    description=f"<:alert:1348340453803687966> An error occurred while unbanning **{user.name}**.",
                    color=0x000000
                )
                await ctx.reply(embed=embed, mention_author=False)
        else:
    ...
    async def _owner_help(self, ctx):
        ...
        embed = Embed(title="Owner Commands",
                      description="`staffadd` ,   `staffremove` ,   `stafflist` , `slist` ,   `getinvite <guild-id>` ,   `getguild <channel-id>` ,   `mutual <user>` ,   `guildban <guild_id> <user_id>` ,   `guildunban <guild_id> <user_id>` ,   `CodeX.restart` ,   `servertour` ,   `forcepurgebots` , `forcepurgeuser` ,   `ownerban` , `bdg add <user> <badge>` ,   `bdg remove <user> <badge>` ,   `global <subcommand>` ,   `np <subcommand>` ,   `autonp <subcommand>`",
                      color=0x000000)
        await ctx.send(embed=embed)
