...
class Ignore(commands.Cog):
  ...
  async def command_add(self, ctx: commands.Context, command_name: str):
      ...
      if not command:
          embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"`{command_name}` is not a valid command.", color=self.color)
      ...
      async with aiosqlite.connect(self.db_path) as db:
          ...
          if result:
              ...
              await db.commit()
              embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully added `{command_name}` to the ignore commands list.", color=self.color)
              await ctx.reply(embed=embed)
  ...
  async def command_remove(self, ctx: commands.Context, command_name: str):
      ...
      async with aiosqlite.connect(self.db_path) as db:
          ...
          result = await cursor.fetchone()
          if not result:
              embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"`{command_name}` is not in the ignore commands list.", color=self.color)
              await ctx.reply(embed=embed)
          else:
              await db.execute("DELETE FROM ignored_commands WHERE guild_id = ? AND command_name = ?", (ctx.guild.id, command_name_normalized))
              await db.commit()
              embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully removed `{command_name}` from the ignore commands list.", color=self.color)
              await ctx.reply(embed=embed)
  ...
  async def channel_add(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(title="<:alert:1348340453803687966> Access Denied", description="You can only add up to 30 channels to the ignore list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"{channel.mention} is already in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_channels (guild_id, channel_id) VALUES (?, ?)", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully added {channel.mention} to the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def channel_remove(self, ctx: commands.Context, channel: discord.TextChannel):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"{channel.mention} is not in the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_channels WHERE guild_id = ? AND channel_id = ?", (ctx.guild.id, channel.id))
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully removed {channel.mention} from the ignore channels list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def user_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if count[0] >= 30:
        embed = discord.Embed(title="<:alert:1348340453803687966> Access Denied", description="You can only add up to 30 users to the ignore list.", color=self.color)
        await ctx.reply(embed=embed)
        return
      ...
      if result:
        embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"{user.mention} is already in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO ignored_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully added {user.mention} to the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def user_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"{user.mention} is not in the ignore users list.", color=self.color)
        await ctx.reply(embed=embed)
      else:
        await db.execute("DELETE FROM ignored_users WHERE guild_id = ? AND user_id = ?", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully removed {user.mention} from the ignore users list.", color=self.color)
  ...
  async def bypass_add(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if result:
        embed = discord.Embed(title="<:cross:1348326385386651748> Error", description=f"{user.mention} is already in the bypass users list.", color=self.color)
        await ctx.reply(embed=embed, mention_author=False)
      else:
        await db.execute("INSERT INTO bypassed_users (guild_id, user_id) VALUES (?, ?)", (ctx.guild.id, user.id))
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully added {user.mention} to the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
  ...
  async def bypass_remove(self, ctx: commands.Context, user: discord.User):
    async with aiosqlite.connect(self.db_path) as db:
      ...
      if not result:
        ...
        await db.commit()
        embed = discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Successfully removed {user.mention} from the bypass users list.", color=self.color)
        await ctx.reply(embed=embed)
