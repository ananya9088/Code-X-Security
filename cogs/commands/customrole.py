...
class Customrole(commands.Cog):
    ...
    async def add_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            ...
            await member.add_roles(role, reason="CodeX Customrole | Role Added")
        else:
    ...
    async def remove_role(self, *, role_id: int, member: discord.Member):
        if member.guild.me.guild_permissions.manage_roles:
            role = discord.Object(id=role_id)
            await member.remove_roles(role, reason="CodeX Customrole | Role Removed")
        else:
            raise discord.Forbidden("Bot does not have permission to manage roles.")
    ...
    async def reset(self, context: Context) -> None:
        if context.author == context.guild.owner or context.author.top_role.position > context.guild.me.top_role.position:
            ...
            description=f"Deleted All Custom Role commands <:tick:1348326381611647046>\n\n**Removed Roles:**\n" + "\n".join(removed_roles) if removed_roles else "No roles were previously set.",
            color=0x000000
        )
    ...
    async def on_message(self, message: discord.Message):
        ...
        if result:
            ...
            if reqrole is None:
                await message.channel.send("<:alert:1348340453803687966> The required role is not set up in this server. Please set it up using `setup reqrole`.")
                return

            
            if reqrole not in message.author.roles:
                await message.channel.send(embed=discord.Embed(description=f"<:alert:1348340453803687966> You need the {reqrole.mention} role to use this command.", color=0x000000))
                return
            ...
            try:
                if role in member.roles:
                    ...
                    await message.channel.send(embed=discord.Embed(
                        title="<:tick:1348326381611647046> Success",
