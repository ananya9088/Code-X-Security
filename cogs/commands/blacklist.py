...
from utils.paginator import Paginator as codex
    ...
    def get_pages(self):
        ...
        for i in range(0, len(self.entries), self.per_page):
            ...
            embed.set_footer(
                ...
                icon_url="https://cdn.discordapp.com/avatars/1368423803440468060/a_f061e6472786781e23bac32fa8d0a667.png?width=115&height=115"
            )
            pages.append(embed)
...
############ FUNCTIONS ############
    ...
    async def blacklistword(self, ctx):
        ...
        paginator = codex(ctx, BlacklistWordPaginator(commands_list).embeds)
    ...
    async def bypass_add(self, ctx, target: Union[discord.Member, discord.Role]):
        ...
        if isinstance(target, discord.Member):
            ...
            if await self.is_user_bypassed(guild_id, target.id):
                embed = discord.Embed(
                    description=f"<:cross:1348326385386651748> | `{target}` is already bypassed.",
                    color=discord.Color.from_rgb(0, 0, 0)
                )
                await ctx.reply(embed=embed)
                return
    ...
    async def command_error(self, ctx, error):
        if isinstance(error, commands.CommandError):
            if not isinstance(error, commands.CommandOnCooldown):
                embed = discord.Embed(
                    description="<:cross:1348326385386651748> | An error occurred while processing the command. Make sure you have **Administrator** permissios.",
                    color=discord.Color.from_rgb(0, 0, 0)
                )
                await ctx.reply(embed=embed)
