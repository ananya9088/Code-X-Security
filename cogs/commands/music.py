...
from core import Cog, CodeX, Context
...
spotify_api = SpotifyAPI(client_id="40c5ee08678d4e60aef46bedc6761fd4", client_secret="2e8ef57a93454b31b035569f64fea7c8`")
...
class MusicControlView(View):
    ...
    async def pause_button(self, interaction: discord.Interaction, button: Button):
        if self.player.paused:
            ...
            await self.player.channel.edit(status=f"<a:musica:1376420282478100511> Playing: {self.player.current.title}")
            button.emoji = "⏸️" 
            await interaction.response.edit_message(view=self)

        elif self.player.playing:
            await self.player.pause(True)
            await self.player.channel.edit(status=f"<a:musica:1376420282478100511> Paused: {self.player.current.title}")
            button.emoji = "⏸️"
            await interaction.response.edit_message(view=self)
...
class Music(commands.Cog):
    ...
    async def display_player_embed(self, player, track, ctx, autoplay=False):
        if track.artwork:
            ...
            embed.add_field(name="Source", value=f"[<:spotify:1348326517507362917> Listen on Spotify]({track.uri})" if "spotify" in track.source else f"[Listen on JioSaavn]({track.uri})" if "jiosaavn" in track.source else f"[Listen on SoundCloud]({track.uri})" if "soundcloud" in track.source else f"[Listen on YouTube]({track.uri})")
    ...
    async def play_source(self, ctx, query):
        if not ctx.author.voice:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> You need to be in a voice channel to use this command.", color=0x000000))
            return
    ...
    async def autoplay(self, ctx: commands.Context):
        vc = ctx.voice_client
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> No song is currently playing.", color=0x000000))
            return

        if not ctx.author.voice or ctx.author.voice.channel.id != vc.channel.id:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> You need to be in the same voice channel as me to use this command.", color=0xFF0000))
            return
        ...
        if vc:
            ...
            await ctx.send(embed=discord.Embed(description=f"<:tick:1348326381611647046>  Autoplay {'enabled' if vc.autoplay == wavelink.AutoPlayMode.enabled else 'disabled'} by {ctx.author.mention}.", color=0x000000))
    ...
    async def pause(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.paused:
            await vc.pause(True)
            await vc.channel.edit(status=f"<a:musica:1376420282478100511> Paused: {vc.current.title}")
            await ctx.send(embed=discord.Embed(description=f"Paused by {ctx.author.mention}.", color=0x000000))
        else:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> Nothing is playing or already paused.", color=0xFF0000))
    ...
    async def resume(self, ctx: commands.Context):
        ...
        if vc and vc.paused:
            ...
            await vc.channel.edit(status=f"<a:musica:1376420282478100511> Playing: {vc.current.title}")
    ...
    async def skip(self, ctx: commands.Context):
        ...
        if vc and vc.playing and not vc.queue.is_empty:
            ...
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> No song is playing or in the queue to skip.", color=0xFF0000))
    ...
    async def volume(self, ctx: commands.Context, level: int):
        ...
        if not vc:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> I'm not connected to a voice channel.", color=0xFF0000))
            return
        ...
        if vc:
            if 1 <= level <= 150:
                ...
                await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> Volume must be between 1 and 150.", color=0xFF0000))
        else:
    ...
    async def queue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> The queue is currently empty.", color=0x000000))
            return
    ...
    async def clearqueue(self, ctx: commands.Context):
        ...
        if not vc or not vc.queue or vc.queue.is_empty:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> No Queue to clear.", color=0xFF0000))
            return
    ...
    async def replay(self, ctx: commands.Context):
        ...
        if not vc or not vc.playing:
            await ctx.send(embed=discord.Embed(description="<:alert:1348340453803687966> I'm not connected to any voice channel.", color=0xFF0000))
            return
    ...
    async def on_wavelink_track_start(self, payload: wavelink.TrackStartEventPayload):
        ...
        if voice_channel:
            await voice_channel.edit(status=f"<a:musica:1376420282478100511> Playing: {track.title}")  # type: ignore
