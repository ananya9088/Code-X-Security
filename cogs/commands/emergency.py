...
""")
    ...
    async def enable(self, ctx):
        CodeX = ['1005088956951564358', '1005088956951564358']
        if ctx.author.id != ctx.guild.owner_id and str(ctx.author.id) not in CodeX:
    ...
    async def emergencysituation(self, ctx):
        ...
        if not await self.is_guild_owner_or_authorised(ctx) and str(ctx.author.id) not in CodeX:
