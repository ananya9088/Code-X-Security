...
class Stats(commands.Cog):
    ...
    async def stats(self, ctx):
        processing_message = await ctx.send("<a:loading:1349238953450147924> Loading CodeX information...")
        ...
        embed = Embed(title="CodeX Statistics: General", color=0x000000)
        embed.add_field(name="<:proton:1348340511303270461> Channels", value=f"Total: **{channel_count}**\nText: **{text_channel_count}**   |   Voice: **{voice_channel_count}**   |   Category: **{category_channel_count}**", inline=False)
        embed.add_field(name="<:signals:1348326407968653312> Uptime", value=f"{uptime}", inline=False)
        embed.add_field(name="<:memberss:1348326396363018290> User Count", value=f"Humans: **{human_count}**   |   Bots: **{bot_count}**", inline=False)
        embed.add_field(name="<:folder:1368993630622318604> Commands", value=f"Total: **{commands_count}**   |   Slash: **{slash_commands}**", inline=False)
        embed.add_field(name="<:Python:1368993967022276728> Libraries Used", value=f"Discord Library: **[discord.py](https://discordpy.readthedocs.io/en/stable/)**\nTotal Libraries: **{total_libraries}**", inline=False)
    ...
    inline=False
        ...
        async def system_button_callback(interaction):
            if interaction.user == ctx.author:
                system_embed = Embed(title="CodeX Statistics: System", color=0x000000)

                system_embed.add_field(name="<:folder:1368993630622318604> System Info", value=f"• Discord.py: **{discord.__version__}**\n• Python: **{platform.python_version()}**\n• Architecture: **{platform.machine()}**\n• Platform: **{platform.system()}**", inline=False)

                system_embed.add_field(name="<:memory2:1368993538427326554> Memory Info", value=f"• Total Memory: **{memory_info.total / (1024 ** 2):,.2f} MB**\n• Memory Left: **{memory_info.available / (1024 ** 2):,.2f} MB**\n• Heap Total: **{memory_info.used / (1024 ** 2):,.2f} MB**", inline=False)
                system_embed.add_field(name="<:cpu:1368993516260425810> CPU Info", value=f"• CPU: **{psutil.cpu_freq().max}' GHz**\n• CPU Usage: **{psutil.cpu_percent()}%**\n• CPU Cores: **{psutil.cpu_count(logical=False)}**\n• CPU Speed: **{cpu_info.current:.2f} MHz**", inline=False)
                system_embed.set_footer(text="Powered by CodeX Development™", icon_url=self.bot.user.display_avatar.url)
        ...
        async def ping_button_callback(interaction):
            if interaction.user == ctx.author:
                ...
                ping_embed.add_field(name="<:signals:1348326407968653312> Bot Latency", value=f"{round(sh.latency * 800)} ms", inline=False)
                ping_embed.add_field(name="<:web_earth1:1348326509907017768> Database Latency", value=f"{db_latency} ms", inline=False)
                ping_embed.add_field(name="<:leaf:1348326420656689182> Websocket Latency", value=f"{wsping} ms", inline=False)
                ping_embed.set_footer(text="Powered by CodeX Development™", icon_url=self.bot.user.display_avatar.url)
        ...
        async def team_button_callback(interaction):
            if interaction.user == ctx.author:
                team_embed = Embed(title="CodeX Team", color=0x000000)
                team_embed.add_field(name="**<:king:1348340519255937045> Bot Owner(s)**", value=">>> **[IDK](https://discord.com/users/1005088956951564358)**,   **[IDK](https://discord.com/users/)**,   **[IDK](https://discord.com/users/)**,   **[IDK](https://discord.com/users/)**", inline=False)
