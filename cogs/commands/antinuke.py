  ...
  async def antinuke(self, ctx, option: str = None):
    ...
    if option is None:
      ...
      if is_activated:
        ...
        setup_embed.description += "\n<:tick:1348326381611647046> | Checking CodeX's role position for optimal configuration..."
        await setup_message.edit(embed=setup_embed)

        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1348326381611647046> | Crafting and configuring the CodeX Supreme role..."
        await setup_message.edit(embed=setup_embed)
        ...
        await asyncio.sleep(1)
        setup_embed.description += "\n<:tick:1348326381611647046> | Ensuring precise placement of the CodeX Supreme role..."
        await setup_message.edit(embed=setup_embed)
        try:
        ...
        embed.set_author(name="CodeX Antinuke", icon_url=self.bot.user.avatar.url)

        embed.set_footer(text="Successfully Enabled Antinuke for this server | Powered by CodeX Development™", icon_url=self.bot.user.avatar.url)
        embed.set_thumbnail(url=self.bot.user.avatar.url)
