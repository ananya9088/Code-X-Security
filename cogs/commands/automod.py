        ...
        async def enable_logging_callback(interaction):
            ...
            try:
                for channel in interaction.guild.channels:
                    if channel.name == "CodeX-automod":
                        await interaction.response.send_message(f"A logging channel with the name \"CodeX-automod\" already exists.", ephemeral=True)
                        return
                log_channel = await interaction.guild.create_text_channel("CodeX-automod", overwrites=overwrites)
