    ...
    async def twitch(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('twitch',)) as existing:
                ...
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:alert:1348340453803687966> Access Denied", description="Twitch notification already set. Remove it first.", color=0x000000))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('twitch', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1348326381611647046> Success", description=f"Twitch notifications set for {role.mention} in {channel.mention}.", color=0x000000))
    ...
    async def youtube(self, ctx, role: discord.Role, channel: discord.TextChannel):
        async with aiosqlite.connect(self.db_path) as db:
            async with db.execute('SELECT * FROM notifications WHERE type = ?', ('youtube',)) as existing:
                row = await existing.fetchone()
                if row:
                    await ctx.reply(embed=discord.Embed(title="<:alert:1348340453803687966> Access Denied", description="YouTube notification already set. Remove it first.", color=0x000000))
                    return

            await db.execute('INSERT INTO notifications (type, role_id, channel_id) VALUES (?, ?, ?)', ('youtube', role.id, channel.id))
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1348326381611647046> Success", description=f"YouTube notifications set for {role.mention} in {channel.mention}.", color=0x000000))
