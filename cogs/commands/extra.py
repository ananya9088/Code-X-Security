...
tick = "<:tick:1348326381611647046>"
cross = "<:cross:1348326385386651748>"
...
class Extra(commands.Cog):
  ...
  async def uptime(self, ctx):
      ...
      embed = discord.Embed(title=f"CodeX Uptime", color=self.color)
      embed.add_field(name="__UTC__", value=f"<:info:1348326490793840640> {uptime_string}\n\n", inline=False)
      embed.add_field(name="__Online Duration__", value=f"<:timer:1348340572586377326> {uptime_duration_string}", inline=False)
  ...
  async def serverinfo(self, ctx):
        ...
        embed.add_field(
            name="**__About__**",
            value=f"**Name : ** {ctx.guild.name}\n**ID :** {ctx.guild.id}\n**Owner <:king:1348340519255937045> :** {ctx.guild.owner} (<@{ctx.guild.owner_id}>)\n**Created At : ** {c_at}\n**Members :** {len(ctx.guild.members)}",
            inline=False
        )
        ...
        if ctx.guild.features:
            features = "\n".join([f"<:tick:1348326381611647046>: {feature[:1].upper() + feature[1:].lower().replace('_', ' ')}" for feature in ctx.guild.features])
            embed.add_field(
                name="**__Features__**",
        ...
        embed.add_field(
            ...
            value=f"Level: {ctx.guild.premium_tier} [<:boost:1348326410682372126> {ctx.guild.premium_subscription_count} boosts]",
            inline=False
        )
...
**Color:** {member.color if member.color else '99aab5'}
  ...
  async def boosts(self, ctx):
    await ctx.send(
      embed=discord.Embed(title=f"<:boost:1348326410682372126> Boosts Count Of {ctx.guild.name}",
  ...
  async def report(self, ctx, *, bug):
    channel = self.bot.get_channel(1385212595379896434)
    ...
    confirm_embed = discord.Embed(title=" <:3742:1363545422752383240> Bug Reported",
