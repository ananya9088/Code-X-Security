...
class Fun(commands.Cog):
  ...
  async def mydog(self, ctx, user: discord.User):
      processing= await ctx.reply("<a:loading:1349238953450147924> Processing Image...")
    ...
    async with aiohttp.ClientSession() as session:
      try:
        async with session.get(f"http://ip-api.com/json/{ip}") as response:
          ...
          embed.set_footer(
            text=f'Made by CodeX Development™',
...
############################
  ...
  async def fake_ban(self, ctx, user: discord.Member):
    embed = discord.Embed(
      ...
      description=f"<:tick:1348326381611647046> | {user.mention} has been successfully banned",
      color=0x000000
    )
    embed.set_footer(
