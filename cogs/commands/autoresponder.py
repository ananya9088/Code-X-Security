    ...
    async def _create(self, ctx, name, *, message):
        ...
        async with aiosqlite.connect(DB_PATH) as db:
            async with db.execute("SELECT COUNT(*) FROM autoresponses WHERE guild_id = ?", (ctx.guild.id,)) as cursor:
                ...
                if count >= 20:
                    return await ctx.reply(embed=discord.Embed(title="<:cross:1348326385386651748> Error!",
            ...
            await db.commit()
            await ctx.reply(embed=discord.Embed(title="<:tick:1348326381611647046> Success",
    ...
    async def _config(self, ctx):
        ...
        if not autoresponses:
            return await ctx.reply(embed=discord.Embed(
                description=f"<:alert:1348340453803687966> | There are no autoresponders in {ctx.guild.name}",
                color=0x000000
            ))
