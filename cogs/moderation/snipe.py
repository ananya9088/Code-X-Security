...
class SnipeView(discord.ui.View):
    ...
    async def send_snipe_embed(self, interaction: discord.Interaction):
        ...
        embed.description = (
            f"<:member:1348326398929932388> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:location:1348326401505361920> **Author ID:** `{snipe['author_id']}`\n"
            f"<:circle2:1348326520581521469> **Author Mention:** <@{snipe['author_id']}>\n"
            f"<:timer:1348340572586377326> **Deleted:** <t:{snipe['deleted_at']}:R>\n"
        )

        if snipe['content']:
            embed.add_field(name="<:delete:1348340540650950816> **Content:**", value=snipe['content'])
        if snipe['attachments']:
            attachment_links = "\n".join([f"[{attachment['name']}]({attachment['url']})" for attachment in snipe['attachments']])
            embed.add_field(name="<:atom:1348326466210889821> **Attachments:**", value=attachment_links)
    ...
    @discord.ui.button(emoji="<:arrow_left:1348340563610435594>", style=discord.ButtonStyle.secondary, custom_id="previous")
    ...
    @discord.ui.button(emoji="<:delete:1348340540650950816>", style=discord.ButtonStyle.danger, custom_id="delete")
    async def delete_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.message.delete()

    @discord.ui.button(emoji="<:arrow_right:1348340445708816494>", style=discord.ButtonStyle.secondary, custom_id="next")
...
class Snipe(commands.Cog):
    ...
    async def snipe(self, ctx):
        ...
        embed.description = (
            f"<:member:1348326398929932388> **Author:** **[{display_name}](https://discord.com/users/{uid})**\n"
            f"<:location:1348326401505361920> **Author ID:** `{first_snipe['author_id']}`\n"
            f"<:circle2:1348326520581521469> **Author Mention:** <@{first_snipe['author_id']}>\n"
            f"<:timer:1348340572586377326> **Deleted:** <t:{first_snipe['deleted_at']}:R>\n"
        )

        if first_snipe['content']:
            embed.add_field(name="<:delete:1348340540650950816> **Content:**", value=first_snipe['content'])
