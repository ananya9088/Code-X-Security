...
class TopCheck(commands.Cog):
    ...
    async def topcheck(self, ctx):
        ...
        "When topcheck is enabled, only users with roles above the bot's (CodeX) role can perform certain moderation actions. "
    ...
    async def topcheck_enable(self, ctx):
        if ctx.author.id != ctx.guild.owner_id:
            return await ctx.reply("<:alert:1348340453803687966> Only the **Server Owner** can enable topcheck.")
        if await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:alert:1348340453803687966> Topcheck is already enabled for this server.")
    ...
    async def topcheck_disable(self, ctx):
        ...
        if not await self.is_topcheck_enabled(ctx.guild.id):
            return await ctx.reply("<:alert:1348340453803687966> Topcheck is not enabled for this server.")
