...
async def do_removal(ctx, limit, predicate, *, before=None, after=None):
    ...
    if len(to_send) > 2000:
        await ctx.send(f"<:tick:1348326381611647046> | Successfully removed {deleted} messages.", delete_after=7)
    else:
...
class Moderation(commands.Cog):
  ...
  async def unlockall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
                  ...
                  if interaction.guild.me.guild_permissions.manage_roles:
                    ...
                    description=f"Unlocking all channels in {ctx.guild.name} .")
                      ...
                      await interaction.channel.send(
                              content=f"<:tick:1348326381611647046> | Successfully Unlocked {a} Channels")   
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                           "<:alert:1348340453803687966> | It seems I'm missing the necessary permissions. Please grant me the `manage roles` permissions and try again.",
                              embed=None,
                                  view=None)
              else:
          ...
          embed5 = discord.Embed(title="<:alert:1348340453803687966> Access Denied",
  ...
  async def lockall(self, ctx):
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
                    ...
                    description=f"Locking all channels in {ctx.guild.name}...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:tick:1348326381611647046> | Successfully locked {a} Channels")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
          ...
          denied = discord.Embed(title="<:alert:1348340453803687966> Access Denied",
              description="Your role should be above my top role.",
              color=0x000000)
  ...
  async def give(self, ctx, member: discord.Member, *, role: discord.Role):
    if not ctx.guild.me.guild_permissions.manage_roles:
        return await ctx.send("<:alert:1348340453803687966> I don't have permission to manage roles!")
    ...
    try:
        ...
        error = discord.Embed(
            color=self.color,
            description="<:alert:1348340453803687966> I don't have permission to manage roles for this user!"
        )
        await ctx.send(embed=error)
    except Exception as e:
        error = discord.Embed(
            color=self.color,
            description=f"<:alert:1348340453803687966> An unexpected error occurred: {str(e)}"
        )
        await ctx.send(embed=error)
                    ...
                    description=f"Hiding all channels in {ctx.guild.name} ...")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:tick:1348326381611647046> | Successfully Hidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
                           "<:alert:1348340453803687966> | It seems I'm missing the necessary permissions. Please grant me the `manage channels` permissions and try again.",
                              embed=None,
                                  view=None)
              else:
                await interaction.response.send_message("Oops! It looks like that message isn't from you. You need to run the command yourself to interact with it.",
                    ...
                    description=f"Unhiding all channels in {ctx.guild.name} .")
                      ...
                      for channel in interaction.guild.channels:
                          try:
                              ...
                              print(e)
                      await interaction.channel.send(
                              content=f"<:tick:1348326381611647046> | Successfully Unhidden {a} Channel(s) .")
                      return
                  else:
                    await interaction.response.edit_message(
                         content=
  ...
  async def _prefix(self, ctx: commands.Context, prefix: str):
      if not prefix:
          await ctx.reply(embed=discord.Embed(title="<:cross:1348326385386651748> Error",
      ...
      if ctx.author == ctx.guild.owner or ctx.author.top_role.position > ctx.guild.me.top_role.position:
          ...
          embed1=discord.Embed(title="<:tick:1348326381611647046> Success",
  ...
  async def changenickname(self, ctx: commands.Context, member: discord.Member, *, name: str = None):
    ...
    try:
        ...
        error = discord.Embed(
            color=self.color,
            description="<:alert:1348340453803687966> | I don't have permission to manage this user's nickname!"
        )
        await ctx.send(embed=error)
    except Exception as e:
        error = discord.Embed(
            color=self.color,
            description=f"<:alert:1348340453803687966> | An error occurred while trying to change the nickname: {str(e)}"
        )
        await ctx.send(embed=error)
    ...
    async def button_callback(interaction: discord.Interaction):
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.manage_channels:
          ...
          await interaction.response.edit_message(
            content=
            "<:alert:1348340453803687966> | It seems I'm missing the necessary permissions. Please grant me the `manage channel` permissions and try again.",
            embed=None,
            view=None)
      else:
        await interaction.response.send_message("Oops! It looks like that message isn't from you. You need to run the command yourself to interact with it.",
        ...
        if len(found_emojis) != 0:
            ...
            for emoji_id in found_emojis:
                try:
                    ...
                    continue  
            await init_message.delete()
            return await ctx.reply(f"<:tick:1348326381611647046> | Successfully deleted {delete_count}/{len(found_emojis)} emoji(s).")
  ...
  async def roleicon(self, ctx: commands.Context, role: discord.Role, *, icon: Union[discord.Emoji, discord.PartialEmoji, str] = None):
    ...
    if isinstance(icon, discord.Emoji) or isinstance(icon, discord.PartialEmoji):
        ...
        try:
            ...
            await role.edit(display_icon=image_data)
            success_embed = discord.Embed(
                description=f"<:tick:1348326381611647046> | Successfully changed the icon for {role.mention}.",
                color=self.color
            )
            return await ctx.reply(embed=success_embed, mention_author=False)
        except Exception as e:
      ...
      if interaction.user == ctx.author:
        if interaction.guild.me.guild_permissions.ban_members:
          ...
          async for idk in interaction.guild.bans(limit=None):
            ...
            a += 1
          await interaction.channel.send(
            content=f"<:tick:1348326381611647046> Successfully Unbanned {a} Members")
        else:
          await interaction.response.edit_message(
            content=
