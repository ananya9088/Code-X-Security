...
class TimeoutView(ui.View):
    ...
    @ui.button(style=discord.ButtonStyle.gray, emoji="<:delete:1348340540650950816>")
...
class ReasonModal(ui.Modal):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        embed = discord.Embed(description=f"**<:member:1348326398929932388> Target User:** [{self.user}](https://discord.com/users/{self.user.id})\n<:circle2:1348326520581521469> **User Mention:** {self.user.mention}\n**<:mail:1348340617935065240> DM Sent:** {dm_status}\n**<:proton:1348340511303270461> Reason:** {reason}", color=0x000000)
        ...
        embed.add_field(name="<:automod2:1348889859632140298> Moderator:", value=interaction.user.mention, inline=False)
...
class Mute(commands.Cog):
    ...
    async def mute(self, ctx, user: discord.Member, time: str = None, *, reason=None):
        ...
        try:
            await user.send(f"<:alert:1348340453803687966> You have been muted in **{ctx.guild.name}** by **{ctx.author}** for {duration_text}. Reason: {reason or 'None'}")
            dm_status = "Yes"
        except discord.Forbidden:
            dm_status = "No"
        except discord.HTTPException:
        ...
        embed = discord.Embed(description=f"**<:member:1348326398929932388> Target User:** [{user}](https://discord.com/users/{user.id})\n"
                                          f"<:circle2:1348326520581521469> **User Mention:** {user.mention}\n"
                                          f"<:mail:1348340617935065240> **DM Sent:** {dm_status}\n"
                                          f"**<:proton:1348340511303270461> Reason:** {reason or 'None'}\n"
                                          f"**<:timer:1348340572586377326> Duration:** {duration_text}",
                              color=self.color)
        embed.set_author(name=f"Successfully Muted {user.name}", icon_url=self.get_user_avatar(user))
        embed.add_field(name="<:automod2:1348889859632140298> Moderator:", value=ctx.author.mention, inline=False)
        embed.set_footer(text=f"Requested by {ctx.author}", icon_url=self.get_user_avatar(ctx.author))
        embed.timestamp = discord.utils.utcnow()
    ...
    async def mute_error(self, ctx, error):
        ...
        if isinstance(error, commands.BotMissingPermissions):
            embed = discord.Embed(title="<:alert:1348340453803687966> Access Denied", description="I don't have permission to mute members.", color=self.color)
            await ctx.send(embed=embed)
        elif isinstance(error, discord.Forbidden):
            embed = discord.Embed(title="<:alert:1348340453803687966> Missing Permissions", description="I can't mute this user as they might have higher privileges (e.g., Admin).", color=self.color)
            await ctx.send(embed=embed)
            
        else:
            embed = discord.Embed(title="<:alert:1348340453803687966> Unexpected Error", description=str(error), color=self.color)
            await ctx.send(embed=embed)
