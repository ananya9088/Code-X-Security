...
class StatusCog(commands.Cog):
    ...
    async def status_task(self):
        ...
        if not self.bot.is_ready():
            ...
            embed.add_field(name="Uptime <a:greencirclepa:1376167246321287249> ", value=uptime_str, inline=False)
            embed.add_field(name="Ping <:ping:1376167777068650589>", value=f"{round(self.bot.latency * 1000)} ms", inline=False)
            embed.add_field(name="Guilds <a:serverupdate:1376167871008477294>", value=str(len(self.bot.guilds)), inline=False)
            embed.add_field(name="Users <:members:1376167929481138196>", value=str(len(self.bot.users)), inline=False)
            embed.add_field(name="Commands <a:command:1376168021234560000>", value=str(len(self.bot.commands)), inline=False)  
            embed.add_field(name="Invite Link <:invite:1378788896967491666>", value="[Click Here](https://discord.com/api/oauth2/authorize?client_id=1362680985497636885&permissions=8&scope=bot)", inline=False)
            embed.add_field(name="Website <:website:1378788763429244968>", value="[Visit Code X](https://codexsecurity.netlify.app/)", inline=False)
